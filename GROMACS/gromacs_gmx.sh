...
for i in {1..9} ; do 
    for folder in run_${i}_design_*_af2pred; do
        echo $folder
        cd "$folder"
        cp ../../../20240418_Scaffolding_myb_2/hits/run_10_design_0_substituted_dldesign_0_cycle1_af2pred/*.mdp .
        if [[ ! -f "run2ns.tpr" ]]; then
            ...
            for i in {1..5}; do 
                ...
                cp *top run_$i/.
                cd run_$i
                sbatch gromacs_submit_cluster_tests.sh "run2ns.tpr" "0"
                cd ..
            done
        wait
