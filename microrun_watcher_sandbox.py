        ...
        if not merged_df.empty:
            ...
            if not filtered_df.iloc[:, 1:].empty:
                ...
                for index, row in filtered_df.iterrows():
                    ...
                    command = f'silentextractspecific ../output/'+ run_number + '/' + run_number + '_input_out.silent ' + row['description'][:-8] + ' > extraction.log'
                    subprocess.run(command, cwd=os.getcwd()+'/hits', shell=True)
