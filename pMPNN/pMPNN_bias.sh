...
while [[ $i -le $max ]];do
    ...
    input_pmpnn=run_${i}/input.silent

    echo "running pmpnn"
    job3=$(sbatch /home/cchacon/protein_design/submit_pMPNN_bias.sh --input_silent "$input_pmpnn" --n_seqs 100 --relax_cycles 0 --bias "$output_bias")
    wait
